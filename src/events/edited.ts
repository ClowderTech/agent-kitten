...
function cutStringAfterPipeOrReturnOriginal(str: string): string {
	...
	// If there is no pipe, return an empty string
	if (index === -1) {
		return str;
	}
...
- Do not misuse bots (e.g., spamming commands, using them for harassment).`;
		...
		const { chat_response } = await chatWithFuncs(
			client.ollama,
			{
				...
				messages: [
					{
						role: "system",
						content: `You are an AI Moderation bot. Your job is to moderate a discord server based on the set rules. If there is an offending message, state its Message ID and the reason for offending seperated by a semicolon \";\" and nothing else. If there are multiple offending messages, seperate them with a comma \",\". If the message does not offend any rules or is generally acceptable, do not ever state its Message ID, any semicolon \";\", or any comma \",\". If you wish to \"Think it out\", you may do so and then afterwards put the vertical line symbol \"|\" so it does not get included in the final result. You must be at least 95% sure it breaks a rule and it must be stated in the rules, no assuming. The rules are as follows:\n\n${rules}`,
					},
					{
						role: "user",
		...
		const response = cutStringAfterPipeOrReturnOriginal(
